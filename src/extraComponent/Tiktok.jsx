...
const Tiktok = () => {
  ...
  // 🔹 Load tiktokFollow status from localStorage
  useEffect(() => {
    const followed = localStorage.getItem("tiktokFollow") === "true";
    setTiktokFollow(followed);
  }, []);
  ...
  const handleClaim = () => {
    ...
    // mark as followed
    localStorage.setItem("tiktokFollow", "true");
    setTiktokFollow(true);
            ...
            <div className="absolute bottom-0 left-0 w-full rounded-t-3xl border-t border-[#40FF54] bg-[#141318] backdrop-blur-md shadow-lg p-6 flex flex-col items-center z-10">
                {!claimed ? (
                    <>
                        ...
                        <button
                            onClick={handleClaim}
                            disabled={tiktokFollow}
                            className={`w-full py-3 rounded-lg text-sm text-white shadow-lg transition ${tiktokFollow
                                    ? "opacity-50 cursor-not-allowed"
                                    : "cursor-pointer hover:opacity-90"
                                }`}
                            style={{ background: "#782AF9" }}
                        >
                            {tiktokFollow ? "Already Claimed" : "Claim Reward"}
                        </button>
                    </>
                ) : (
