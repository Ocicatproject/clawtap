...
const DAILY_TIME = 24 * 60 * 60 * 1000; // 24 hours
...
const Dailyreward = () => {
  ...
  useEffect(() => {
    const checkClaimStatus = () => {
      const savedTime = localStorage.getItem("dailyReward");
      ...
      const lastClaim = new Date(savedTime).getTime();
      if (isNaN(lastClaim)) {
        setCanClaim(true);
        setTimeLeft("");
        return;
  ...
  const handleClaim = () => {
    ...
    // Save claim time (ISO format)
    localStorage.setItem("dailyReward", new Date().toISOString());
      ...
      <div className="absolute bottom-0 left-0 w-full rounded-t-3xl border-t border-[#40FF54] bg-[#141318] backdrop-blur-md shadow-lg p-6 flex flex-col items-center z-10">
        {!claimed ? (
          <>
            ...
            <p className="text-[20px] font-bold text-[#E5E5E5] pt-5 opacity-90">
              Daily reward
            </p>
          ...
          <div className="flex flex-col items-center py-10">
            <img src={copyicon} alt="Success" className="w-12 h-12 mb-4" />
