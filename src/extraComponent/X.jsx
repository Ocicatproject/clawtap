...
const X = () => {
    ...
    // 🔹 Load xFollow status from localStorage
    useEffect(() => {
        const followed = localStorage.getItem("xFollow") === "true";
        setXFollow(followed);
    }, []);
    ...
    const handleClaim = () => {
        ...
        // mark as followed
        localStorage.setItem("xFollow", "true");
        setXFollow(true);
                ...
                {!claimed ? (
                    <>
                        ...
                        <button
                            onClick={handleClaim}
                            disabled={xFollow}
                            className={`w-full py-3 rounded-lg text-sm text-white shadow-lg transition ${xFollow
                                    ? "opacity-50 cursor-not-allowed"
                                    : "cursor-pointer hover:opacity-90"
                                }`}
                            style={{ background: "#782AF9" }}
                        >
                            {xFollow ? "Already Claimed" : "Claim Reward"}
                        </button>
                    </>
                ) : (
