...
const InstaGram = () => {
  ...
  // 🔹 Load instaFollow status from localStorage
  useEffect(() => {
    const followed = localStorage.getItem("instaFollow") === "true";
    setInstaFollow(followed);
  }, []);

  // 🎁 Claim handler
  const handleClaim = () => {
    if (instaFollow) return;
    ...
    // mark as followed
    localStorage.setItem("instaFollow", "true");
    setInstaFollow(true);
            ...
            <div className="absolute bottom-0 left-0 w-full rounded-t-3xl border-t border-[#40FF54] bg-[#141318] backdrop-blur-md shadow-lg p-6 flex flex-col items-center z-10">
                {!claimed ? (
                    <>
                        ...
                        <button
                            onClick={handleClaim}
                            disabled={instaFollow}
                            className={`w-full py-3 rounded-lg text-sm text-white shadow-lg transition ${instaFollow
                                    ? "opacity-50 cursor-not-allowed"
                                    : "cursor-pointer hover:opacity-90"
                                }`}
                            style={{ background: "#782AF9" }}
                        >
                            {instaFollow ? "Already Claimed" : "Claim Reward"}
                        </button>
                    </>
                ) : (
