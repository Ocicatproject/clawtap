...
const Ocicat = () => {
  ...
  // 🔹 Load ocicatFollow status from localStorage
  useEffect(() => {
    const followed = localStorage.getItem("ocicatFollow") === "true";
    setOcicatFollow(followed);
  }, []);

  // 🎁 Claim handler
  const handleClaim = () => {
    if (ocicatFollow) return;
    ...
    // mark as followed
    localStorage.setItem("ocicatFollow", "true");
    setOcicatFollow(true);
      ...
      <div className="absolute bottom-0 left-0 w-full rounded-t-3xl border-t border-[#40FF54] bg-[#141318] backdrop-blur-md shadow-lg p-6 flex flex-col items-center z-10">
        {!claimed ? (
          <>
            ...
            >
              {ocicatFollow ? "Already Claimed" : "Claim Reward"}
            </button>
          </>
        ) : (
